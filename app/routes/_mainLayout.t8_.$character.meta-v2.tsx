...
export const loader = async ({ params }: DataFunctionArgs) => {
  ...
  const sheetName = `${character}-meta-v2`
...
export default function Index() {
  ...
  const keyMovesTable: TableData | undefined = metaTables.find(
    t => t.name === 'key_moves',
  )

  const keyMoves = keyMovesTable
    ? keyMovesTable.rows.map(r => ({
        command: r[0],
        description: r[1],
      }))
    : undefined
  ...
  return (
    <>
      ...
      <ContentContainer enableBottomPadding>
        ...
        {keyMoves && <KeyMoves moves={keyMoves} />}
        {tables.map(table => {
