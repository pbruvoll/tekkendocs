 ...
 */
export const sheetWeakFormatToSections = (sheetData: string[][], tableHeaders: string[]): SheetSection[] => {
  const sections: SheetSection[] = []
  const tableHeadersLowerCase = tableHeaders.map(t => t.toLocaleLowerCase());
  let currentRow = 0;
  let currentColumn = 0;
  while(true) {
    const tableHeading = findTableHeading(sheetData, tableHeadersLowerCase, currentRow, currentColumn);
    if(!tableHeading) {
      break;
    }
    const numColumns = getNumColumns(sheetData, tableHeading);
    const tableData = parseTableData(sheetData, tableHeadersLowerCase, tableHeading.i +1 , tableHeading.j, numColumns);
    if(tableData.length) {
      sections.push({
        rows: tableData,
...
const findTableHeading = (sheetData: string[][], tableHeadersLowerCase: string[], startRow: number, startColumn: number) => {
  let i = startRow;
  let j = startColumn;
  while(true) {
...
function getNumColumns(sheetData: string[][], tableHeading: { i: number; j: number; value: string }) {
...
function parseTableData(sheetData: string[][], tableHeadersLowerCase: string[], startRow: number, startColumn: number, numColumns: number) {
